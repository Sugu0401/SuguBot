...
async def displayembed(ctx):
    embed = discord.Embed(
        title = "Hola, ¿quién soy?",
        description = "Soy SuguBot, un bot desarrollado para satisfascer\n las necesidades en psicología crew, mi meta es expandirme lo máximo posible y sacarle el mayor provecho a la API de LoL.",
        color = 0xceb7f2
    )
    ...
    embed.add_field(name="Conóceme", value="https://twitter.com/matiassctt\nhttps://www.instagram.com/matiassctt/", inline=True)
...
async def perfil(ctx, member: discord.Member = None):#Tomas como parametro la tag del usuario
    ...
    embed.set_author(name=f"Información de: {member}")
    ...
    embed.add_field(name="Tu cuenta se creó el:", value=member.created_at.strftime("%#d %B %Y a las %I:%M %p UTC"), inline=True)
    embed.add_field(name=f"Te uniste al servidor el:", value=member.joined_at.strftime("%#d %B %Y a las %I:%M %p UTC"), inline=True)
    embed.add_field(name="¿Sabías qué?", value=random.choice(facts), inline=True)
...
parametros = ()      
###############Vault#####################    
@client.command(name="vaultsave", help="Guarda una imagen, nombre y después url")
async def vaultsave(ctx, nombre, url):
        ...
        if validacion=="gif":
            ...
            for x in a:
                ...
                if nombre==x:
                      c=c+1
            if c>=1:
                response="El nombre de esa imagen ya existe, usa uno diferente." ##usa otro o mira la lista, pelotudo
        ...
        if validacion=="jpg" or validacion=="png" or validacion=="jpeg":
            ...
            for x in b:
                ...
                if nombre==x:
                      c=c+1
            if c>=1:
                response="El nombre de esa imagen ya existe, usa uno diferente."
                await ctx.send(response)
            else:         
    ...
    if verify=="False":
        ...
        file.close()
        response = "¡Parece que es la primera vez que tu servidor usa nuestro Vault! Por eso tuvimos que usar esta primera vez para crear un directorio para que tus imágenes no se pierdan, por favor vuelve a subir tu primera imagen o gif :)"
        await ctx.send(response)

    print ("Comando usado: Vaultsave")
@client.command(name="vaultsend", help="Envía una imagen")
...
async def vaultlist(ctx):
    ...
    contents = contents + "**Listado de imágenes:\n**"
    d = f.readlines()
    for i in d:
        i=i.replace("\n", "")
        if i!=" " or h!="":
...
async def vaultdelete(ctx): #agregar nombre cuando vuelva a funcionar
    quote = [
    'El delete ahora mismo no está funcionando, llámalo a Mati para que borre la imagen manualmente'
    ]
...
@client.command(name="lol", help="Partida de LoL, sólo nombre")
async def lol(ctx, nombre, region="la2"):
    ...
    if body==404:
        ...
        acu=acu + "\nPuedes ver mas de la partida en: " + url
    await ctx.send(acu)
...
async def loltier(ctx, rol):
    ...
    if rol!="adc" and rol!="sup" and rol!="mid" and rol!="top" and rol!="jg":
        response="Rol incorrecto, prueba con support, adc, mid, jg o top"
    await ctx.send(response)
...
@client.command(name="profile", help="Perfil de lol, nombre y región.")
async def profile(ctx, nombre, region="la2"):
    ...
    if region!="na1" and region!="la1" and region !="la2":
        await ctx.send("Lo siento :( las únicas regiones programadas son na1, la1 y la2")
        return 
        ...
        for x in info:
            if info[c]["queueType"]!="RANKED_FLEX_5x5":
                ...
                try:
                    ...
                    promo = "Este jugador no está en promo"
            c=c+1
        total=wins+losses
        winr=(wins/total)*100
        embed = discord.Embed(
            title = f"Perfil de {nombre}",
            description=f"Puedes ver más de tu información en {url}",
...
async def calc(ctx, number_one, ope, number_two):
    if ope=="+":
        ...
        response="Parámetro no encontrado o faltaron los espacios entre números" ##o no sabes poner numeros
    await ctx.send(response)
...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game(name='-h, Ayudando a '+str(len(client.guilds))+' servidores | Conectada con '+str(len(set(client.get_all_members())))+' usuarios :)'))
...
async def hug(ctx, member: discord.Member = None):
    if member is None:
        await (ctx.send("¡Menciona a alguien para abrazarle!"))
        return
    avatar=discord.Embed(
        colour=member.color,
...
async def kiss(ctx, member: discord.Member = None):
    if member is None:
        await (ctx.send("¡Menciona a alguien para besarle!"))
        return
    avatar=discord.Embed(
        colour=member.color,
...
async def reset(ctx):
    if ctx.message.author.id==186955944410742785:
        ...
        await ctx.send("Este comando sólo lo puede usar Mati, tagealo")
...
async def join(ctx):
    ...
    await ctx.send(f"Me uní a {channel}")
...
async def leave(ctx):
    ...
    if voice and voice.is_connected():
        ...
        await ctx.send("Al parecer no estoy en un canal de voz :(")
...
async def init(ctx, *url: str):
    ...
    try:
        ...
        await ctx.send("¡Méteme a un canal de voz con -j!") #decía metete
        return
    ...
    try:
        ...
        await ctx.send("¡Ya estás escuchando música! Pon -p para agregar más canciones a la queue")
        return
    ...
    await ctx.send("Amigo, te pido por favor que me esperes porque soy un botsito nuevo y me cuesta bastante")
...
async def resume(ctx):
    ...
    if voice and voice.is_paused():
        ...
        await ctx.send("La música no esta pausada")
...
async def stop(ctx):
    ...
    if voice and voice.is_playing():
        ...
        await ctx.send("No hay música que detener")
...
async def play(ctx, *url: str):
    ...
    try:
        ...
        await ctx.send("¡Méteme a un canal de voz con -j!")
        return
...
async def skip(ctx):
    ...
    if voice and voice.is_playing():
        ...
        voice.stop()
        await ctx.send("Reproduciendo si es que hay otra canción...")
    else:
...
async def help(ctx, a=""):
    a=a.lower()
    if a=="":
        await ctx.send("Para mas información escribe -help [nombre del comando]")
        author = ctx.message.author
        embed = discord.Embed(
            title = "¡Mis comandos!",
            color = discord.Color.dark_blue(),
            description = "Puedes encontrar información más detallada en https://sugubot.000webhostapp.com/"
        )
        embed.set_thumbnail(url="https://i.imgur.com/E2jPb6o.png")
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        ...
        embed.add_field(name="Ocio:", value="**-8ball**, **-coin**, **-pick**, **-hugemoji**, **-hug**, **-kiss**, **-calc**, **-tusa**, **-cancion**", inline=False)
        ...
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Nombre de invocador, Region*", inline=False)
        embed.add_field(name="*", value="Automáticamente en LA2, puede ser LA1 o NA.")
        await ctx.send(embed=embed)
    elif a=="ugg":
        embed=discord.Embed(
            title="League of Legends",
            color = 0xceb7f2,
            description = "Te envío la información del campeón que quieras",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Nombre del campeón", inline=False)
        await ctx.send(embed=embed)
    elif a=="tier":
        embed=discord.Embed(
            title="League of Legends",
            color = 0xceb7f2,
            description = "Te envío un link de los mejores campeones en ese rol",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Rol", inline=False)
        await ctx.send(embed=embed)
    elif a=="profile":
        embed=discord.Embed(
            title="League of Legends",
        ...
        embed=discord.Embed(
            title="Vault",
            color = 0xceb7f2,
            description = "Envío el archivo a tu servidor",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Nombre del archivo", inline=False)
        await ctx.send(embed=embed)
    elif a=="vaultsave":
        embed=discord.Embed(
            title="Vault",
        ...
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Nombre del archivo, URL del archivo*", inline=False)
        ...
        embed=discord.Embed(
            title="Música",
            color = 0xceb7f2,
            description = "Te pongo una canción.*",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Nombre o URL de la cancion", inline=False)
        ...
        embed=discord.Embed(
            title="Música",
            color = 0xceb7f2,
            description = "Le pongo pausa a la canción.",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    elif a=="resume":
        embed=discord.Embed(
            title="Música",
            color = 0xceb7f2,
            description = "Resumo la canción pausada.",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    elif a=="stop":
        embed=discord.Embed(
            title="Música",
            color = 0xceb7f2,
            description = "Termino con toda la música del servidor.",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    elif a=="skip":
        embed=discord.Embed(
            title="Música",
            color = 0xceb7f2,
            description = "Pongo la siguiente canción.",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    elif a=="perfil":
        ...
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Usuario*", inline=False)
        ...
        embed=discord.Embed(
            title="Ocio",
            color = 0xceb7f2,
            description = "Lanzo una moneda, que puede ser cara o cruz.",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    elif a=="8ball":
        ...
        embed=discord.Embed(
            title="Ocio",
            color = 0xceb7f2,
            description = "Si no sabes a quién elegir, yo lo hago por vos ;).",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Parámetros que quieras elegir*", inline=False)
        ...
        embed=discord.Embed(
            title="Ocio",
            color = 0xceb7f2,
            description = "Te envío el emoji en grande y en linda calidad.",
        )
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Emoji", inline=False)
        await ctx.send(embed=embed)
    elif a=="prune":
        embed=discord.Embed(
            title="Moderacion",
        ...
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Cantidad", inline=False)
        embed.add_field(name="*", value="Por seguridad, sólo los usuarios que puedan manejar los mensajes pueden usar este comando.", inline=False)
        await ctx.send(embed=embed)
    elif a=="kick":
        embed=discord.Embed(
            title="Moderacion",
        ...
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Usuario, Razon", inline=False)
        embed.add_field(name="*", value="Sólo los usuarios que puedan kickear a otros pueden usar este comando.", inline=False)
        await ctx.send(embed=embed)
    elif a=="ban":
        embed=discord.Embed(
            title="Moderación",
        ...
        embed.set_footer(text=f"Requerido por {ctx.author}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Parámetros:", value="Usuario, Razón, Cantidad de días que borro sus mensajes", inline=False)
        embed.add_field(name="*", value="Sólo los usuarios que puedan banear a otros pueden usar este comando.", inline=False)
        await ctx.send(embed=embed)
    else:
...
async def status(ctx, msj="status"):
    if ctx.message.author.id==186955944410742785:
        ...
        await ctx.send("Sólo Mati puede cambiar el status del bot")
...
async def kick(ctx, member: discord.Member, reason=None):
    try:
        ...
        await ctx.send(f"El usuario {member} fue kickeado por {ctx.message.author} por la razón de **{reason}**")
    except:
        await ctx.send("No cuentas con los permisos necesarios para realizar esta acción")
...
async def ban(ctx, member: discord.Member,*,reason=None, delete_message_days=1):
    try:
        ...
        await ctx.send(f"El usuario {member} fue baneado por {ctx.message.author} por la razón de **{reason}** y se borraron los mensajes de hace {delete_message_days} dias")
    except:
        await ctx.send("No cuentas con los permisos necesarios para realizar esta acción")
...
async def clear(ctx, amount=0):
    ...
    if amount>=1 and amount<=50:
        ...
        await ctx.send(response)
    if amount>=51:
        response="Más de 50 es mucho, ¿no?"
        await ctx.send(response)
        
@client.command(name="hugemoji")
async def hugemoji(ctx, emoji: discord.Emoji):
    a=str(ctx.message.author.guild.id)
    if os.path.exists("hugemojis/"+a+emoji.name+".jpg")==False:
        img_data = requests.get(emoji.url).content
        with open("hugemojis/"+a+emoji.name+".jpg", 'wb') as handler:
            handler.write(img_data)
    if os.path.exists("ultramojis/"+a+emoji.name+".jpg")==False:
        r = requests.post(
        "https://api.deepai.org/api/waifu2x",
        data={
            'image': str(emoji.url),
        },
        headers={'api-key': 'API DE WAIFU2X'}
        )
        body = json.dumps(r.json())
        body = json.loads(body)
        img_data = requests.get(body["output_url"]).content
        with open("ultramojis/"+a+emoji.name+".jpg", 'wb') as handler:
            handler.write(img_data)
    await ctx.send(file=discord.File("hugemojis/"+a+emoji.name+".jpg"))
    await ctx.send(file=discord.File("ultramojis/"+a+emoji.name+".jpg"))
@client.command(name="gigaprune")
async def gigaprune(ctx):
    await ctx.send("ASAAAAAAAAAAAAAAAAAAA, TE ASUSTASTE")
client.run(token)
